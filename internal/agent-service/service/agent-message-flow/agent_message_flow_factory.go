...
func NewAgentMessageFlow() *compose.Graph[*request.AgentChatContext, []*schema.Message] {
	//创建图信息
	graph := compose.NewGraph[*request.AgentChatContext, []*schema.Message](compose.WithGenLocalState(func(ctx context.Context) (state *AgentState) {
		return &AgentState{}
	}))
	//增加图节点
...
func AddAgentFlowNode(graph *compose.Graph[*request.AgentChatContext, []*schema.Message]) {
	...
	_ = graph.AddLambdaNode(keyOfPromptVariables, compose.InvokableLambda[*request.AgentChatContext, map[string]any](promptVars.AssemblePromptVariables))
